...
if __name__ == "__main__":
    ...
    experiment_id="testRETEST_"+dataset_name
    ...
    load_from_pixel_list_path = "" #"TEST_CIFAR-10_random_1553144498.pkl" #"TEST_CIFAR-10_LIME_1552602173.pkl" #make sure to load test set
...
#INITAL TEST of MODEL
    ...
    test_x_deteriated = np.copy(test_x)
    ...
    test_x_deteriated = model_instance.CheckInputArrayAndResize(test_x_deteriated,model_instance.min_height,model_instance.min_width)

    num_pixels = dataset_json["image_y"] * dataset_json["image_x"]
    
    num_pixels_in_padded = test_x_deteriated[0].shape[:-1][0] * test_x_deteriated[0].shape[:-1][1]
...
#EXPLAIN and PRODUCE ORDERED PIXEL LISTS
    ...
    if(generate_random_pixel_list):
        ...
        pixel_lists = GenerateRandomPixelWeights(test_x_deteriated.shape)
        if(save_pixel_list):
                print("saving pixel lists")
                SavePixelList("TEST_"+dataset_name,explanation_name,pixel_lists)
        ...
        if(load_from_pixel_list_path != ""):
            ...
            if(normalise_data):
                pixel_lists = CreatePixelListForAllData(dataset_tool.StandardizeImages(test_x_deteriated), test_y, dataset_name, model_instance, explanation_instance,additional_args=None,framework_tool=framework_tool, train_x=dataset_tool.StandardizeImages(train_x), train_y=train_y, denormalise_function=denormalise_function)
            else:
                pixel_lists = CreatePixelListForAllData(test_x_deteriated, test_y, dataset_name, model_instance, explanation_instance,additional_args=None,framework_tool=framework_tool, train_x=train_x, train_y=train_y)
...
# EVALUATION
    ...
    for deterioration_step in range(num_deterioration_steps):
        deterioration_output_path = str(experiment_id)+"_"+explanation_name+"_"+format(deterioration_step, '05d')+"_deteriation_results.csv"
        ...
        x_deteriated = PerturbData(test_x_deteriated,deterioration_rate,pixel_lists,deterioration_step,deterioration_index_step,perturbation_function)
        
        #RETEST PHASE
        x_prediction_probs, x_predictions = model_instance.Predict(dataset_tool.StandardizeImages(test_x_deteriated), return_prediction_scores = True)
        ...
        new_accuracy = model_instance.EvaluateModel(dataset_tool.StandardizeImages(test_x_deteriated), test_y, model_train_params["batch_size"])
        print("metrics", model_instance.model.metrics_names)
